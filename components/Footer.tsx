...
export const Footer: React.FC = () => {
  const { data: footer, loading } = useContent('*[_type == "footer"][0]');
  const radius = 30;
    ...
    <footer className="py-20 px-6 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto flex flex-col items-center relative z-10">
        ...
        <div className="flex gap-8 border-t border-white/10 pt-8 w-full justify-center">
          <a href={content.linkedin} target="_blank" rel="noopener noreferrer" className="text-bone/40 hover:text-matcha transition-colors duration-300">
            <Linkedin size={20} strokeWidth={1.5} />
          </a>
          <a href={`mailto:${content.email}`} className="text-bone/40 hover:text-matcha transition-colors duration-300">
