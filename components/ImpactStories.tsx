...
export const ImpactStories: React.FC = () => {
  const { data: cmsStories, loading } = useContent('*[_type == "impactStory"] | order(order asc)');

  if (loading) return null;

  const displayStories = cmsStories?.length > 0 ? cmsStories : [
    {
      id: '1',
      story: "Juliana no solo diagnostica patologías; diagnostica contextos. Su capacidad para entender la cosmovisión Wayúu y traducirla a protocolos médicos salvó vidas que el sistema daba por perdidas.",
      author: "María Epieyu",
    ...
    {
      id: '2',
      story: "Rigor científico impecable con una humanidad desarmante. En el laboratorio es precisa; en cuidados paliativos es una presencia que sana incluso cuando no hay cura.",
    ...
    {
      id: '3',
      story: "Ella transforma la incertidumbre de la sala de urgencias en un plan de acción claro. Tiene ese 'pulso' que no se enseña en las facultades.",
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {displayStories.map((story: any, index: number) => (
            <GlassCard key={story._id || index} className="p-8 rounded-2xl flex flex-col h-full" delay={index * 0.1}>
              ...
              <div className="mt-auto flex items-center gap-4 pt-6 border-t border-white/5">
                {story.image ? (
                  <img
                    src={story.image?.asset ? urlFor(story.image).url() : story.image}
                    alt={story.author}
